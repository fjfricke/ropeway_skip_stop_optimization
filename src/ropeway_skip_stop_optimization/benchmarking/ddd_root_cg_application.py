...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=False,
        help="Run bounded exact-CG branch dives after the root solve.",
    )
    parser.add_argument("--dive-time-limit", type=float, default=300.0)
    parser.add_argument("--dive-node-time-limit", type=float, default=60.0)
    parser.add_argument("--dive-count", type=int, default=4)
    parser.add_argument("--dive-max-depth", type=int, default=12)
    parser.add_argument("--dive-node-iterations", type=int, default=20)
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Per-call CP-SAT budget for complete Passenger-dual-guided trajectory "
            "batches; zero disables this primal-only channel."
        ),
    )
    parser.add_argument("--coordinated-primal-interval", type=int, default=1)
    parser.add_argument("--coordinated-primal-workers", type=int, default=8)
    parser.add_argument("--coordinated-primal-candidates", type=int, default=1)
    parser.add_argument(
...
def run_namespace(
    ...
    *,
    ...
    prepared_fixed_k: DddPreparedFixedKArcFlowRun | None = None
    ...
    if args.example is None:
        ...
        if (
            bool(getattr(args, "fixed_k_certified", False))
            and args.fleet_mode == "fixed_starts"
            and fixed_start_policy is DddFixedKStartPolicy.BALANCED_REFERENCE
        ):
            prepared_fixed_k = prepare_ddd_fixed_k_arc_flow_run(
                DddFixedKArcFlowRunConfig(
                    ...
                    start_policy=fixed_start_policy,
                    start_layout_time_limit_seconds=float(
                        getattr(args, "cp_seed_time_limit", 30.0)
                    ),
                    total_time_limit_seconds=float(args.total_time_limit or 600.0),
                )
            )
            scenario = prepared_fixed_k.scenario
            artifact = prepared_fixed_k.problem.artifact
        else:
        ...
        if (
            prepared_fixed_k is None
            and fixed_start_policy is DddFixedKStartPolicy.CANONICAL_ROPE
        ):
        ...
        if prepared_fixed_k is not None:
            pass
        elif args.fleet_mode == "optimized_initial_placement":
            builder = replace(
                builder,
                fleet_config=replace(
                    builder.fleet_config,
    ...
    if prepared_fixed_k is not None:
        trajectory_problem = prepared_fixed_k.problem.resolved_trajectory_problem
    elif args.fleet_mode == "reservoir_dispatch":
        ...
        maximum_visit_count = (
            math.ceil(
                (args.warmup_seconds + core.operational_end_seconds + minimum_duration)
                / minimum_duration
            )
            + 1
        )
            ...
            start_domain=DddReservoirTrajectoryStartDomain(
                ...
                boundary=DddReservoirBoundaryConfig(
                    ...
                    dispatch_resource_usages=tuple(
                        ...
                        for resource_id in sorted(set(args.reservoir_dispatch_resource))
                    ),
    ...
    if prepared_fixed_k is not None:
        passenger_build = prepared_fixed_k.problem.passenger_build
    elif args.fleet_mode == "reservoir_dispatch":
        ...
        passenger_build = EanPassengerCandidateBuilder().build(
            scenario,
            artifact,
        )
    objective = EanPassengerObjective(args.objective)
    fixed_k_problem = None if prepared_fixed_k is None else prepared_fixed_k.problem
    if getattr(args, "fixed_k_certified", False) and fixed_k_problem is None:
    ...
    default_checkpoint_path = args.output_dir / f"{output_stem}.checkpoint.json"
    checkpoint_path = (
        None
    ...
    seed_result = None
    if (
        fixed_k_problem is not None
        and resume_state is None
        and initial_trajectories is None
    ):
        ...
        seed_result = DddFixedKSeedCoordinator(
            cp_sat_time_limit_seconds=float(getattr(args, "cp_seed_time_limit", 30.0)),
            cp_sat_num_workers=int(getattr(args, "cp_seed_workers", 1)),
        ).solve(
            problem,
            evaluate=lambda solution: (
                seed_evaluator.evaluate(problem, solution).objective_value
            ),
        )
    ...
    def progress(iteration: DddTrajectoryRootCgIteration) -> None:
        ...
        pricing_constraints = sum(
            item.model_linear_constraint_count + item.model_general_constraint_count
    ...
    remaining_root_budget = (
        ...
        if args.total_time_limit is None
            ...
            - (
                float(getattr(args, "dive_time_limit", 0.0))
                if bool(getattr(args, "dive", False))
                else 0.0
            )
            - (perf_counter() - application_started),
        )
    )
    solver = DddTrajectoryExactRootColumnGenerationSolver(
        ...
        certified_fixed_k_mode=bool(getattr(args, "fixed_k_certified", False)),
        objective_floor=float(getattr(args, "objective_floor", 0.0)),
        restricted_mip_interval=int(getattr(args, "restricted_mip_interval", 1)),
        restricted_mip_time_limit_seconds=getattr(
            args, "restricted_mip_time_limit", None
        ),
        final_mip_time_limit_seconds=float(getattr(args, "final_mip_time_limit", 0.0)),
    )
    root_solve_started = perf_counter()
    if (
        seed_result is not None
        and seed_result.status is not DddFixedKSeedStatus.FEASIBLE
    ):
        status_by_seed = {
        ...
        result = DddTrajectoryRootCgResult(
            ...
            certificate_valid=(
                seed_result.status is not DddFixedKSeedStatus.INTERNAL_VALIDATION_ERROR
            ),
            objective_floor=float(getattr(args, "objective_floor", 0.0)),
        )
    else:
        result = solver.solve(
            ...
            boundary_occurrences=(
                ...
                else fixed_k_problem.boundary_context.resource_occurrences
            ),
        )
        result = replace(
            result,
            total_seconds=(
                result.total_seconds + root_solve_started - application_started
            ),
            seed_kind=(
                result.seed_kind
    ...
    dive_payload = None
    if bool(getattr(args, "dive", False)) and result.trajectories:
        remaining_application_budget = (
            float(getattr(args, "dive_time_limit", 300.0))
            if args.total_time_limit is None
            else min(
                float(getattr(args, "dive_time_limit", 300.0)),
                max(
                    1e-6,
                    args.total_time_limit - (perf_counter() - application_started),
                ),
            )
        )

        def dive_progress(event: object) -> None:
            if args.no_progress:
                return
            upper = getattr(event, "best_upper_bound")
            upper_text = "-" if upper is None else f"{upper:,.1f}"
            print(
                f"dive={getattr(event, 'dive_index') + 1:02d} "
                f"depth={getattr(event, 'depth') + 1:02d} "
                f"phase={getattr(event, 'phase')} "
                f"LB={getattr(event, 'global_lower_bound'):,.1f} "
                f"UB={upper_text} "
                f"left={getattr(event, 'remaining_seconds'):.0f}s"
            )

        dive_result = DddTrajectoryDiveCoordinator(
            DddTrajectoryDiveConfig(
                maximum_depth=int(getattr(args, "dive_max_depth", 12)),
                maximum_dives=int(getattr(args, "dive_count", 4)),
                total_time_limit_seconds=remaining_application_budget,
                node_time_limit_seconds=min(
                    float(getattr(args, "dive_node_time_limit", 60.0)),
                    remaining_application_budget,
                ),
                node_maximum_iterations=int(getattr(args, "dive_node_iterations", 20)),
                pricing_time_limit_tiers_seconds=(
                    tuple(getattr(args, "pricing_time_limit_tier", ()))
                    or (float(args.pricing_time_limit),)
                ),
                restricted_mip_time_limit_seconds=float(
                    getattr(args, "restricted_mip_time_limit", None) or 15.0
                ),
                final_mip_time_limit_seconds=float(
                    getattr(args, "final_mip_time_limit", 0.0) or 30.0
                ),
            )
        ).solve(
            problem=problem,
            ...
            progress_callback=dive_progress,
        )
        dive_payload = {
            "status": dive_result.status.value,
            "certified_root_lower_bound": (dive_result.certified_root_lower_bound),
            "best_validated_upper_bound": (dive_result.best_validated_upper_bound),
            "relative_gap": dive_result.relative_gap,
            "step_count": len(dive_result.steps),
            "steps": [asdict(item) for item in dive_result.steps],
            "total_seconds": dive_result.total_seconds,
            "detail": dive_result.detail,
        }
        result = replace(
            result,
            best_upper_bound=dive_result.best_validated_upper_bound,
            relative_gap=dive_result.relative_gap,
            trajectories=dive_result.trajectories,
            incumbent_option_ids=dive_result.incumbent_option_ids,
            incumbent_ride_values_by_id=(dive_result.incumbent_ride_values_by_id),
            total_seconds=perf_counter() - application_started,
        )
    payload = {
        "schema_version": 3,
        ...
        "experimental": not bool(getattr(args, "fixed_k_certified", False)),
        ...
        "warmup_seconds": (
            args.warmup_seconds if args.fleet_mode == "reservoir_dispatch" else None
        ),
        ...
        "fixed_k_certified": bool(getattr(args, "fixed_k_certified", False)),
        "fixed_start_policy": getattr(
            args, "fixed_start_policy", DddFixedKStartPolicy.LEGACY.value
        ),
        "operating_mode": (
            None if fixed_k_problem is None else fixed_k_problem.operating_mode.value
        ),

...
def with_exit_waiting(prepared: DddPreparedFixedKArcFlowRun, *, maximum_seconds: float,
                      step_seconds: float) -> DddPreparedFixedKArcFlowRun:
    ...
    if problem.resolved_trajectory_problem.waiting_policy.domain is not DddTrajectoryWaitingDomain.NO_WAIT:
    ...
    config = replace(problem.artifact.config, station_configs=tuple(
        replace(s, waiting_mode=StationWaitingMode.END_OF_PLATFORM_WAIT,
                max_wait_seconds=maximum_seconds, fifo_capacity=None)
    ...
    trajectory = EanArtifactToDddMovementProblemAdapter(waiting_step_seconds=step_seconds).build_trajectory_problem(artifact)
    movement = trajectory.structural_movement_problem
    if movement.starts != base.starts:
        raise ValueError('exit waiting must preserve the exact fixed starts and visit bounds')
    ...
    for option in movement.route_options:
        ...
        if (option.duration_tick, option.platform_entry_offset_seconds, option.platform_exit_offset_seconds) != (
            old.duration_tick, old.platform_entry_offset_seconds, old.platform_exit_offset_seconds):
        ...
        for usage in option.resource_usages:
            ...
            if usage.resource_id in added_resources:
                ...
                if usage.leader_clear_offset_tick + usage.separation_after_tick(resource) > option.duration_tick:
                    raise ValueError('exit-wait boundary needs a longer prefix history')
            else:
                prior = old_usages[usage.resource_id]
                if (usage.follower_enter_offset_tick, usage.leader_clear_offset_tick, usage.separation_after_tick(resource)) != (
                    prior.follower_enter_offset_tick, prior.leader_clear_offset_tick,
                    prior.separation_after_tick(base.resources_by_id[usage.resource_id])):
                    raise ValueError('exit-wait lift changed existing boundary resource timing')
    ...
    for state in problem.boundary_context.initial_states:
        ...
        elif state.kind is EanInitialPlacementStateKind.ROPE and state.previous_service is not None:
            decision = DddRouteDecision.STOP if state.previous_service else DddRouteDecision.SKIP
        else:
            raise ValueError('cannot reconstruct fixed prefix for exit-wait boundary')
        options = [o for o in movement.route_options if o.from_state_id == state.switch_id and o.decision is decision]
        if len(options) != 1:
        ...
        for usage in option.resource_usages:
            ...
            if clear + usage.separation_after_tick(resource) > 0:
                boundary.append(DddReferenceResourceOccurrence(
                    resource_id=usage.resource_id, cabin_id=state.cabin_id, visit_index=-1,
                    leader_clear_time_seconds=ddd_tick_to_seconds(clear),
                    follower_enter_time_seconds=ddd_tick_to_seconds(entry),
                    separation_after_seconds=usage.separation_after_seconds, boundary_origin=True))
    lifted = replace(problem, artifact=artifact, trajectory_problem=trajectory,
        passenger_build=EanPassengerCandidateBuilder().build(scenario, artifact),
        boundary_context=replace(problem.boundary_context, resource_occurrences=tuple(boundary)))
    lifted.validate()
    # Rebuild resource occurrences of the optional seed against the new core.
    seeds = ()
    if prepared.seed_trajectories:
        seeds = solution_from_cp_sat_payload(lifted, {'trajectory_supports': [
            {'cabin_id': t.cabin_id, 'route_option_ids': [v.route_option_id for v in t.visits],
             'switch_times_tick': [ddd_seconds_to_tick(v.switch_time_seconds) for v in t.visits],
             'wait_ticks': [ddd_seconds_to_tick(v.wait_seconds) for v in t.visits]}
            for t in prepared.seed_trajectories]}).trajectories

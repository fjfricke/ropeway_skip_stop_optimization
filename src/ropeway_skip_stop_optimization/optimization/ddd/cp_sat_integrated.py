...
from .cp_sat_certificate import (
    FORMULATION_VERSION, DddCpSatIncumbent, stable_fingerprint,
    validate_ddd_cp_sat_domain, validate_ddd_cp_sat_incumbent,
    solution_from_cp_sat_payload, write_ddd_cp_sat_checkpoint,
)
from .cp_sat_movement import DddCpSatMovementModel, build_ddd_cp_sat_movement
from .cp_sat_passenger import DddCpSatCostEncoding, DddCpSatPassengerModel, build_ddd_cp_sat_passengers
...
class DddIntegratedCpSatConfig:
    ...
    def validate(self) -> None:
        ...
        if type(self.num_workers) is not int or self.num_workers <= 0 or type(self.seed) is not int or self.seed < 0:
...
def _movement_values(problem: DddFixedKTrajectoryProblem, built: DddCpSatMovementModel,
                     solution: DddReferenceSolution) -> tuple[dict[int, int], dict[tuple[int, int], int]]:
    ...
    for cabin, states in built.states_by_cabin.items():
        ...
        for index, state in enumerate(states):
            ...
            time = ddd_seconds_to_tick(visits[index].switch_time_seconds) if active else terminal
            ...
            for option in movement.route_options_by_state_id[state]:
                ...
                values[built.selection_by_key[cabin, index, option.id].index] = int(selected)
            stop = unique_stop_route_option(movement, state, error_context="CP-SAT hints")
            alight_times[cabin, index] = time + ddd_seconds_to_tick(stop.platform_entry_offset_seconds)
...
def build_ddd_integrated_cp_sat(
    ...
    *,
    ...
    if domain_manifest is not None and stable_fingerprint(manifest) != stable_fingerprint(domain_manifest):
    ...
    if fixed_movement is not None:
        checked = validate_ddd_cp_sat_incumbent(problem, fixed_movement, {}, provenance="fixed movement")
    ...
    passengers = build_ddd_cp_sat_passengers(problem, built, cost_encoding=cost_encoding,
                                            deadline_monotonic=deadline_monotonic)
    ...
    return DddIntegratedCpSatModel(built, passengers, manifest, scope,
        stable_fingerprint({"version": FORMULATION_VERSION, "model": str(built.model.proto)}), stats)
...
class DddIntegratedCpSatResult:
    ...
    def cp_lower_bound(self) -> float | None:
        return None if self.cp_best_bound_tick is None else self.cp_best_bound_tick / DDD_TIME_TICKS_PER_SECOND
    ...
    def relative_gap(self) -> float | None:
        ...
        return None if ub is None or ub == 0 or lb is None else max(0.0, (ub - lb) / abs(ub))
    ...
    def to_payload(self) -> dict:
        ...
        payload.update({
            ...
            "absolute_gap": None if self.validated_upper_bound is None or self.cp_lower_bound is None
                else max(0.0, self.validated_upper_bound - self.cp_lower_bound),
            "incumbent": None if self.incumbent is None else self.incumbent.to_payload(),
            # Existing Arc-Flow runners consume these timetable fields along
            # with the canonical metadata added by the benchmark runner.
            "trajectory_supports": [] if self.incumbent is None else self.incumbent.to_payload()["trajectory_supports"],
            "served_count": None if self.incumbent is None else sum(self.incumbent.ride_counts.values()),
            "unserved_count": None if self.incumbent is None else sum(self.incumbent.unserved_counts.values()),
        })
        return payload
...
def _raw_incumbent(built: DddIntegratedCpSatModel, value: Callable) -> dict:
    ...
    for cabin, states in built.movement.states_by_cabin.items():
        ...
        supports.append({"cabin_id": cabin, "route_option_ids": routes, "switch_times_tick": ticks})
    return {"trajectory_supports": supports,
        "ride_counts": {q: int(value(y)) for q, y in built.passengers.ride_count.items() if value(y)},
...
def _validate_raw(problem: DddFixedKTrajectoryProblem, raw: dict) -> DddCpSatIncumbent:
    return validate_ddd_cp_sat_incumbent(problem, solution_from_cp_sat_payload(problem, raw), raw["ride_counts"],
        provenance="integrated_cp_sat", expected_objective_tick=raw["objective_tick"])
...
class _IncumbentCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        try:
            ...
            self.events.append({"elapsed_seconds": elapsed, "kind": "cp_incumbent",
                "objective_tick": self.raw["objective_tick"], "raw_bound_tick": self.best_objective_bound})
            improves_saved = self.saved_upper_tick is None or self.raw["objective_tick"] <= self.saved_upper_tick
            if self.config.checkpoint_path is not None and improves_saved and elapsed - self.last_checkpoint >= self.config.checkpoint_interval_seconds:
                before = perf_counter()
                incumbent = _validate_raw(self.problem, self.raw)
                write_ddd_cp_sat_checkpoint(self.config.checkpoint_path, problem=self.problem,
                    manifest=self.built.domain_manifest, incumbent=incumbent)
    ...
    def solve(
        ...
        *,
        ...
        if primal_seed is not None:
            incumbent = validate_ddd_cp_sat_incumbent(problem, primal_seed.solution, primal_seed.ride_counts,
                provenance=primal_seed.provenance, expected_objective_tick=primal_seed.objective_tick)
        if fixed_movement is not None:
            fixed_checked = validate_ddd_cp_sat_incumbent(problem, fixed_movement, {}, provenance="fixed")
        ...
        try:
            built = build_ddd_integrated_cp_sat(problem, cost_encoding=self.config.cost_encoding,
                fixed_movement=fixed_movement, deadline_monotonic=deadline, domain_manifest=manifest)
        except TimeoutError:
            pass
        if built is not None and incumbent is not None:
            values, alight_times = _movement_values(problem, built.movement, incumbent.solution)
            for index, value in values.items():
                built.movement.model.add_hint(built.movement.model.get_int_var_from_proto_index(index), value)
            built.passengers.add_hints(problem, built.movement.model, incumbent.ride_counts, alight_times)
        ...
        if built is not None and perf_counter() < deadline:
            ...
            def record_bound(bound):
                ...
                if elapsed - last_sample[0] >= 0.5:
                    events.append({"elapsed_seconds": elapsed, "kind": "cp_bound", "raw_bound_tick": bound})
                    last_sample[0] = elapsed
            solver.best_bound_callback = record_bound
            callback = _IncumbentCallback(problem, built, self.config, started, events,
            ...
            if callback.error is not None:
                raise RuntimeError("CP-SAT callback validation failed") from callback.error
            status_name = solver.status_name(status)
            response = solver.response_stats()
            termination = "OPTIMAL" if status == cp_model.OPTIMAL else (
                "TIME_LIMIT" if perf_counter() >= deadline else "SOLVER_STOPPED"
            ) if status in (cp_model.FEASIBLE, cp_model.UNKNOWN) else status_name
            ...
            if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                before = perf_counter()
                cp_incumbent = _validate_raw(problem, _raw_incumbent(built, solver.value))
                validation_seconds += perf_counter() - before
                cp_objective = cp_incumbent.objective_tick
                if incumbent is None or cp_incumbent.objective_tick <= incumbent.objective_tick:
                    incumbent = cp_incumbent
            raw_bound = float(solver.best_objective_bound) if status != cp_model.INFEASIBLE else None
            ...
            if status == cp_model.OPTIMAL:
                ...
                if cp_objective is None or abs(raw_bound - cp_objective) > 0.25:
                    raise RuntimeError("CP-SAT optimal status has inconsistent objective/bound")
                cp_bound = cp_objective
                optimal = True
            if incumbent is not None and cp_bound is not None and cp_bound > incumbent.objective_tick:
                raise RuntimeError("CP-SAT lower bound exceeds validated upper bound")
            events.append({"elapsed_seconds": perf_counter() - started, "kind": "final",
                "cp_objective_tick": cp_objective, "cp_bound_tick": cp_bound, "status": status_name})
        ...
        if incumbent is not None and self.config.checkpoint_path is not None:
            before = perf_counter()
            write_ddd_cp_sat_checkpoint(self.config.checkpoint_path, problem=problem, manifest=manifest, incumbent=incumbent)
            validation_seconds += perf_counter() - before
        horizon = problem.resolved_trajectory_problem.movement_core.passenger_service_end_tick
        constant = sum(g.count * max(0, horizon - ddd_seconds_to_tick(g.release_time_seconds)) for g in problem.passenger_build.demand_groups)
        return DddIntegratedCpSatResult(problem.fingerprint, manifest, scope,
            None if built is None else built.model_fingerprint,
            status_name, termination, optimal, incumbent, cp_objective, cp_bound, raw_bound, constant,
            self.config.cost_encoding.value, build_seconds, solve_seconds, validation_seconds,
            perf_counter() - started, {} if built is None else built.stats, tuple(events), response,
            None if primal_seed is None else primal_seed.provenance, self.config.num_workers, self.config.seed)

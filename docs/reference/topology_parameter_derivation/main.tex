...
Doppelmayr reports a general cabin height of 2.22~m and a cabin length of
2.656~m for its ten-passenger OMEGA IV-10 SI D cabin \cite{doppelmayrDLine}.
The artificial case uses that published height and rounds the longitudinal
length conservatively to 3.0~m.
...
Cabin height $H_C$ & 2.22 m & manufacturer reference \\
...
\cite[Sec.~6.3.5]{en12929,strmtgRM2}.  Collision geometry must not use the
reduced pendulum length of a dynamics model.  Such a reduced length depends on
mass, centre of gravity, and moment of inertia \cite{wenin2019}; it is not the
distance to the outer cabin envelope.

Let $g_{\mathrm{roof}}$ denote the vertical distance from the attachment point
to the cabin roof and let $H_C$ be the cabin height.  The relevant geometric
drop to the lowest point of the upright cabin envelope is
\begin{equation}
z_{\mathrm{env}}=g_{\mathrm{roof}}+H_C.
\label{eq:envelope-drop}
\end{equation}
For a rectangular longitudinal section, exact rigid-body projection at sway
angle $\theta_R$ gives
\begin{equation}
d_{\mathrm R}^{\mathrm{rect}}
 =L_C\cos(\theta_R)+2z_{\mathrm{env}}\sin(\theta_R)+c_R.
\label{eq:rect-swing-spacing}
\end{equation}
Until a carrier CAD envelope is available, the artificial case replaces
$L_C\cos(\theta_R)$ by the larger $L_C$.  Its deliberately conservative
symmetric-envelope approximation is therefore
\begin{equation}
d_{\mathrm R}^{\mathrm{swing}}
  = L_C+2z_{\mathrm{env}}\sin(\theta_R)+c_R.
...
\end{equation}
The standard fixes neither $g_{\mathrm{roof}}$ nor $c_R$:
$g_{\mathrm{roof}}$ is carrier-specific and $c_R$ is a design safety allowance.
The French guidance uses an additional
...
$c_R=0.5$~m and $g_{\mathrm{roof}}=2.0$~m as explicit experimental
assumptions.  With the manufacturer-reported $H_C=2.22$~m this gives
$z_{\mathrm{env}}=4.22$~m.  For $L_C=3$~m and
$\vlinehaul=6$~m/s, Eq.~\eqref{eq:swing-spacing} yields
\[
d_{\mathrm R}^{\mathrm{swing}}\approx6.31\ \mathrm m,
\qquad
\hlinehaul^{\mathrm{swing}}\approx1.05\ \mathrm s.
\]
The exact rectangular expression in Eq.~\eqref{eq:rect-swing-spacing} would
give approximately 1.02~s.  Neither the illustrative 2~m roof offset nor the
resulting 1.05~s is a literature-backed operating value.  The calculation
demonstrates only that the familiar 60~m pitch cannot be justified by
adjacent-cabin sway alone.  Exact carrier CAD envelopes may replace
Eq.~\eqref{eq:swing-spacing} by
\[
...
Thus it sets $h_R=h_R^{\mathrm{swing}}=1.05$~s conditionally; it does not claim
that 1.05~s is certified for a present installation.
...
longitudinal sway angle and $c_M$ an additional clearance.  The same geometric
drop $z_{\mathrm{env}}$ is used for the conservative swept-envelope distance
and its time equivalent:
\begin{equation}
d_M^{\mathrm{env}}
 =L_C+2z_{\mathrm{env}}\sin(\theta_M^{\mathrm{emergency}})+c_M,
\qquad
...
$H_C$ & 2.22 m & manufacturer cabin geometry \\
$g_{\mathrm{roof}}$ & 2.0 m & experimental compact urban hanger geometry \\
$z_{\mathrm{env}}$ & 4.22 m & derived from $g_{\mathrm{roof}}+H_C$ \\
...
$L_C=3$~m, $H_C=2.22$~m, $g_{\mathrm{roof}}=2.0$~m, $\theta_R=
\theta_M^{\mathrm{emergency}}=0.34$~rad, $c_R=c_M=0.5$~m, and
$v_R=v_M=6$~m/s.  It gives
\[
h_R=h_M^{\mathrm{env}}\simeq1.05\ \mathrm{s}.
\]
...
\text{A:}\quad&H_{r,r'}=9.00\ \mathrm{s}
  &&\forall r,r',\\
\text{B:}\quad&H_{B,B}=H_{B,S}=1.05\ \mathrm{s},\\
&H_{S,B}=H_{S,S}=4.98\ \mathrm{s},
  &&g_B=6.03\ \mathrm{s}.
\\[-0.25em]
\text{C:}\quad&H_{r,r'}=1.05\ \mathrm{s}
  &&\forall r,r',\qquad g_C=2.10\ \mathrm{s}.
\end{align*}
...
\begin{itemize}[leftmargin=*]
  \item $c_S$, $c_R$, $c_M$, $H_C$, and $g_{\mathrm{roof}}$;
...
\hservice &= \frac{L_C+c_S}{\vstation},\\
z_{\mathrm{env}} &= g_{\mathrm{roof}}+H_C,\\
\hlinehaul &= \frac{L_C+2z_{\mathrm{env}}\sin(\theta_R)+c_R}{\vlinehaul},\\
 h_M^{\mathrm{env}}
 &=\frac{L_C+2z_{\mathrm{env}}\sin(\theta_M^{\mathrm{emergency}})+c_M}{v_M},\\
\tau_M^{\mathrm{stop}}
...
The frontend likewise does not reconstruct collision clearance from the legacy
quantity $L_C+c_{\mathrm{legacy}}$.  The complete exported policy carries the
derived spatial values $d_R^{\mathrm{swing}}$ and $d_S=L_C+c_S$.  Its continuous
replay checker applies the value belonging to each rope, bypass, or service
motion interval and minimizes separation over the complete interval, not only
at rendered frames.  As an independent consistency guard, it recalculates these
two spatial values from the exported base geometry and reports an indeterminate
result if scenario and policy disagree.  This prevents a stale artifact from
silently certifying a replay after $H_C$ or $g_{\mathrm{roof}}$ changes.
...
tests for the common reference inputs must obtain
$z_{\mathrm{env}}=4.22$~m, $h_R=1.05$~s,
$h_S=11.67$~s, architecture-A entries of 9~s, the architecture-B matrix and
6.03~s slot, and architecture-C main-rope entries of 1.05~s.  Synthetic C
...
provide the exact carrier envelope and attachment geometry, permissible sway,

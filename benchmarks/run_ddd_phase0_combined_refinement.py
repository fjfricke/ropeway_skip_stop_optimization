...
def main() -> None:
    output_dir = Path("benchmarks/output/ddd_phase0_combined_refinement")
    ...
    if (
        ...
        raise RuntimeError(f"DDD combined refinement failed: {result.status}")
    ...
    payload = {
        ...
        "instance_id": THREE_STATION_NETWORK_COMBINED_CASE_ID,
    ...
    output_path = output_dir / f"{THREE_STATION_NETWORK_COMBINED_CASE_ID}.json"
    ...
    for iteration in result.iterations:
        print(
            ...
            f"prefix_variables={iteration.prefix_variable_count} "
            f"rows={iteration.constraint_count} "
            f"conflict_rows={iteration.conflict_constraint_count} "
            f"lb={iteration.global_lower_bound} "
            f"ub={iteration.global_upper_bound} "
            f"violations={iteration.conflict_count} "
            f"added_cuts={len(iteration.added_cut_ids)} "
            f"split={iteration.split_state_id}@{iteration.split_boundary_seconds}"
        )
    print(
        f"{result.status.value}: LB={result.global_lower_bound}; "
        f"UB={result.global_upper_bound}; cuts={len(result.conflict_cuts)}; "

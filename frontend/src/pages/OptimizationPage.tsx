...
export default function OptimizationPage() {
  ...
  useEffect(() => {
    fetch(`/generated/optimization/index.json?t=${Date.now()}`, { cache: "no-store" })
      ...
      .then(setIndex)
      .catch((cause) => setError(cause instanceof Error ? cause.message : "Index unavailable"));
  }, []);
  return (
      ...
      <section className="campaign-grid">
        {index?.campaigns.map((campaign) => (
          <AppLink className="campaign-card" href={`/optimization/${campaign.campaign_id}`} key={campaign.campaign_id}>
            ...
            <strong>{campaign.completed_trial_count ?? 0} / {campaign.trial_count ?? 0} trials</strong>
          </AppLink>
        ))}
      </section>
    </main>

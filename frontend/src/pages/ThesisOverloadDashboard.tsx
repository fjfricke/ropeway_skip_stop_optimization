  ...
  const charts = useMemo(() => {
    ...
    return {
      ...
      bounds: [
        ...
        ...(ref ? [{ name: "All-Stop-Startwert", color: "#e56a22", dashed: true, points: line(ref.ub) }] : []),
      ] satisfies Series[],
      gap: [{ name: "Lexikografischer Gap", color: progressColors[3], step: true, points: extend(detail.points.map(p => ({ x: p.seconds, y: p.gap_percent })), elapsed) }] satisfies Series[],
    };
  }, [detail]);
  ...
  return <div className="greedy-dashboard overload-dashboard">
    ...
    <section className="optimization-panel">
      ...
      <dl className="greedy-facts overload-facts">
        <div><dt>Status</dt><dd>{detail.status.replaceAll("_", " ")}</dd></div>
        ...
        <div><dt>Journey Time</dt><dd>{hasNativeIncumbent ? `${fmt(latest.journey_time_seconds)} Personen-s` : "—"}</dd></div>
    ...
    {!movementOnly && <section className="optimization-panel">
      <div className="panel-heading"><div><p className="eyebrow">Solverfortschritt</p><h2>Lower Bound, Upper Bound und Gap</h2></div><span>Exaktes lexikografisches Ziel: Unserved, dann Journey Time</span></div>
      <div className="greedy-two-charts"><Curve series={charts.bounds} xMax={charts.elapsed} xLabel="Sekunden" yLabel="Lexikografischer Zielwert" zero={false}/><Curve series={charts.gap} xMax={charts.elapsed} xLabel="Sekunden" yLabel="Gap · %"/></div>
      <p className="greedy-note">UB und LB gelten für den kombinierten ganzzahligen Zielwert. {hasReference ? "Die gestrichelte All-Stop-Linie ist ein Vergleichswert und keine globale Schranke. " : ""}Journey Time besitzt deshalb keine separate native LB.</p>

...
export default function JourneyCampaignDashboard({ campaign }: { campaign: OptimizationCampaignSnapshot }) {
  ...
  const seeded = jobs.some(j => j.all_stop_start != null);
  return <>
    <section className="optimization-panel">
      ...
      {seeded && <p>Restart with a validated All-Stop MIP start, including passenger assignment. Starts are inherited results; only later improvements are new search progress.</p>}
    ...
    {([['reference', 'All-Stop calibration'], ['relative', 'Relative demand · 25% / 75%'], ['constant', 'Constant demand · 50% of K30']] as const).map(([kind, title]) => <section className="optimization-panel" key={kind}>
      ...
      <div style={{ overflowX: 'auto' }}><table className="pattern-screening__table">
        <thead><tr><th>Case</th><th>K</th><th>Mode</th><th>{kind === 'reference' ? 'Feasible N' : 'Demand'}</th>{kind !== 'reference' && <>{seeded && <th>All-Stop start</th>}<th>Native incumbent</th><th>Confirmed Journey Time</th><th>Lower bound</th><th>Gap</th></>}<th>Status</th><th>Progress</th></tr></thead>
        <tbody>{jobs.filter(j => j.kind === kind && j.status !== 'deferred').map(job => <tr key={job.id}>
          ...
          {kind !== 'reference' && <>{seeded && <td>{number(job.all_stop_start)}</td>}<td>{number(job.native_incumbent)}</td><td>{number(job.validated_objective)}</td><td>{number(job.lower_bound)}</td><td>{job.gap == null ? '—' : `${(job.gap * 100).toFixed(2)}%`}</td></>}

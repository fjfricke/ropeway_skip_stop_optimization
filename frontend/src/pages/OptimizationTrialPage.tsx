...
export default function OptimizationTrialPage({ campaignId, policyId, fleetCount }: { campaignId: string; policyId: string; fleetCount: number }) {
  ...
  return <main className="optimization-shell"><header className="optimization-hero"><div><p className="eyebrow">Active trial{trial.formulation ? ` · ${trial.formulation.replaceAll("_", " ")}` : ""}</p><h1>{policyId} · K={fleetCount}</h1></div><div className="campaign-status"><span className={`run-state run-state--${optimizationStatusClass(semanticStatus)}`}>{semanticStatus.replaceAll("_", " ")}</span>{stale && <span className="stale-badge">stale</span>}<strong>{usesSolverTime ? `${(trial.elapsed_seconds ?? 0).toFixed(0)}s` : `round ${trial.round_index ?? 0}`}</strong></div></header><section className="metric-strip"><Metric label="Certified LB" value={formatOptimizationNumber(trial.certified_lower_bound, 3)} /><Metric label="Validated UB" value={formatOptimizationNumber(trial.validated_upper_bound, 3)} /><Metric label="Global gap" value={trial.relative_gap == null ? "—" : `${(trial.relative_gap * 100).toFixed(3)}%`} /><Metric label="Elapsed" value={`${(trial.elapsed_seconds ?? 0).toFixed(1)}s`} /><Metric label="Stage" value={trial.stage ?? "between rounds"} /></section>{trial.start_layout_kind && <section className="metric-strip"><Metric label="Start layout" value={trial.start_layout_kind.replaceAll("_", " ")} /><Metric label="K max AS" value={String(trial.all_stop_maximum_cabin_count ?? "—")} /><Metric label="Candidates" value={formatInteger(trial.start_layout_candidate_count)} /><Metric label="Served stations" value={formatInteger(trial.start_layout_service_station_count)} /><Metric label="Min station stops" value={formatInteger(trial.start_layout_minimum_station_stop_count)} /><Metric label="Max service gap" value={formatOptimizationDuration(trial.start_layout_maximum_service_gap_seconds)} /></section>}<section className="optimization-panel"><div className="panel-heading"><h2>Global certificate timeline</h2><span>Only globally valid bounds are shown</span></div><BoundProgressChart xLabel={usesSolverTime ? "Elapsed seconds" : "CG round"} points={samples.map((event) => ({ x: usesSolverTime ? (event.elapsed_seconds ?? 0) : (event.round_index ?? 0), lower: event.global_certified_lower_bound!, upper: event.global_validated_upper_bound ?? null }))} /></section><section className="optimization-panel"><div className="panel-heading"><h2>Semantic event stream</h2><span>{trial.events.length} events</span></div><EventList events={trial.events.slice(-30).reverse()} /></section></main>;
}
